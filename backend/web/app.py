...
from backend.web.flask_auth import (
    session_csrf_mutation_guard, session_principal_resolver,
    install_session_auth, create_auth_blueprint,
)
    ...
    if situation_api is not None:
        app.register_blueprint(
            create_situation_blueprint(
                api=situation_api, principal_resolver=principal_resolver, mutation_guard=mutation_guard
            )
        )
    if catalog_api is not None:
        app.register_blueprint(
            create_catalog_blueprint(
                api=catalog_api, principal_resolver=principal_resolver, mutation_guard=mutation_guard
            )
        )
    if indicator_api is not None:
        app.register_blueprint(
            create_indicator_blueprint(
                api=indicator_api, principal_resolver=principal_resolver, mutation_guard=mutation_guard
            )
        )
