...
const emergencyResponseLabel = (v) => (EMERGENCY_RESPONSE_OPTIONS.find(([k])=>k===v)||[v,'未设置'])[1];
...
function airportNumber(id){const match=/^AP(\d+)$/i.exec(String(id||''));return match?match[1].padStart(3,'0'):String(id||'—')}
...
function detailAirports(x){
  ...
  const runways=a.runways==null?'<div class="field-note">跑道结构数据未知。</div>'
    :(a.runways.length?`<div class="runway-fact-list">${a.runways.map(r=>`<article class="runway-fact"><strong>${esc(r.runway_id)}</strong>${kv([['长度',r.length_m!=null?`${r.length_m} m`:'—'],['宽度',r.width_m!=null?`${r.width_m} m`:'—'],['道面',r.surface],['灯光',r.lighted==null?'未知':r.lighted?'有':'无'],['低端',r.low_end?.ident],['高端',r.high_end?.ident]])}</article>`).join('')}</div>`:'<div class="field-note">已确认无跑道。</div>');
  const protections=`<ol class="protection-list">${AIRPORT_PROTECTION_MEASURES.map(item=>`<li>${esc(item)}</li>`).join('')}</ol><p class="field-note">标准化基础资料条目，不计入运行保障评分。</p>`;
  const ops=p?`${kv([['配置完整',p.configuration_complete?'是':'否'],['容量/窗',p.capacity_per_window],['综合保障等级',p.support_level],['应急处理能力',emergencyResponseLabel(p.emergency_response_level)]])}
    <p class="field-note">应急处理能力表示机场受损后场道及关键设施的抢修恢复能力。</p>
  ...
  return paneTabs('basic')
    + `<section class="detail-section" data-airport-section="basic"><h3>基本信息</h3>${kv([['编号',airportNumber(a.airport_id)],['名称',a.airport_name],['设施类型',a.facility_type],['机场角色',roleLabel(a.role)],['ICAO',a.icao_code],['IATA',a.iata_code],['区域',regionDisplayWithCode(a.region)],['城市',a.municipality],['经纬度',`${formatCoordinate(a.longitude)}, ${formatCoordinate(a.latitude)}`],['高程 m',formatDecimal(a.elevation_m)],['定期服务',a.scheduled_service?'是':'否'],['实际机位',formatInteger(a.parking_stand_count)],['可用空域','全空域（仿真假设）']])}</section>`
    + `<section class="detail-section" data-airport-section="basic"><h3>跑道</h3>${runways}</section>`
    + `<section class="detail-section" data-airport-section="basic"><h3>机场防护措施</h3>${protections}</section>`
...
function editorAirports(create){
  ...
  const basic=`<div class="edit-grid">${create?'':inputField('技术编号','edAirportId',a.airport_id,'text',false,'readonly')}${inputField('名称','edAirportName',a.airport_name)}${selectField('设施类型','edFacilityType',[['large_airport','大型机场'],['medium_airport','中型机场'],['small_airport','小型机场']],a.facility_type||'medium_airport')}${selectField('机场角色','edAirportRole',[['military','军用'],['joint','军民合用'],['civil','民用']],a.role||'military')}${inputField('ICAO','edIcao',a.icao_code)}${inputField('IATA','edIata',a.iata_code)}${inputField('区域','edRegion',a.region)}${inputField('城市','edMunicipality',a.municipality)}${inputField('经度','edLon',a.longitude,'number',false,'step="any"')}${inputField('纬度','edLat',a.latitude,'number',false,'step="any"')}${inputField('高程 m','edElevation',a.elevation_m,'number',false,'step="any"')}${inputField('实际机位','edParkingStandCount',a.parking_stand_count,'number',false,'min="0"')}${selectField('定期服务','edScheduled',[['true','是'],['false','否']],String(a.scheduled_service??false))}</div><div class="static-assumption"><strong>可用空域</strong><span>全空域（仿真假设）</span></div><div class="form-section"><div class="form-section-head"><strong>机场防护措施</strong><span class="field-note">标准化基础资料条目，不参与评分</span></div><ol class="protection-list">${AIRPORT_PROTECTION_MEASURES.map(item=>`<li>${esc(item)}</li>`).join('')}</ol></div>`;
...
function renderEditor(){
  ...
  refs.detail.innerHTML=`<div class="editor-scroll">${html}<div id="editorMessage" class="inline-message hidden"></div></div><div class="editor-footer"><button id="editorCancel" class="btn ghost" type="button">取消</button><button id="editorSave" class="btn primary" type="button">保存</button></div>`;
...
function airportPayload(){const id=$('edAirportId')?.value.trim()||'';const runwaysKnown=$('edRunwaysKnown').checked;const runways=runwaysKnown?[...refs.detail.querySelectorAll('.runway-row')].map(r=>({runway_id:r.querySelector('.rw-id').value.trim(),length_m:num(r.querySelector('.rw-length').value),width_m:num(r.querySelector('.rw-width').value),surface:r.querySelector('.rw-surface').value.trim()||null,lighted:r.querySelector('.rw-lighted').value===''?null:r.querySelector('.rw-lighted').value==='true',low_end:runwayEnd(r,'low'),high_end:runwayEnd(r,'high')})):null;const runwayCount=runways===null?null:runways.length;const lengths=(runways||[]).map(r=>r.length_m).filter(v=>v!=null).map(Number);return {airport:{airport_id:id,airport_name:$('edAirportName').value.trim(),facility_type:$('edFacilityType').value,role:$('edAirportRole').value,icao_code:nullableText('edIcao'),iata_code:nullableText('edIata'),region:nullableText('edRegion'),municipality:nullableText('edMunicipality'),longitude:num($('edLon').value),latitude:num($('edLat').value),elevation_m:num($('edElevation').value),scheduled_service:$('edScheduled').value==='true',parking_stand_count:int($('edParkingStandCount').value),runway_count:runwayCount,max_runway_length_m:lengths.length?Math.max(...lengths):null,runways},operational_profile:{airport_id:id,configuration_complete:$('edConfigComplete').checked,capacity_per_window:int($('edCapacity').value),support_level:nullableText('edSupportLevel'),emergency_response_level:$('edEmergencyResponseLevel').value||null,aircraft_support:rows('.support-row',r=>({aircraft_type_id:r.querySelector('.row-aircraft').value,initial_quantity:int(r.querySelector('.row-initial').value),tau_reset_windows:int(r.querySelector('.row-reset').value)})),resource_stocks:rows('.stock-row',r=>({resource_type_id:r.querySelector('.row-resource').value,initial_quantity:num(r.querySelector('.row-stock').value),replenishment_capacity_per_window:num(r.querySelector('.row-replenish').value)}))}};}
