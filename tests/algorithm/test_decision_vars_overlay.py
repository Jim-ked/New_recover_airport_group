...
class DecisionVarsOverlayTests(unittest.TestCase):
    ...
    def test_paths_arriving_before_mission_window_are_excluded_without_waiting(self):
        ds, rp = self._input()
        ds["static"]["missions"][0]["_duty_window"] = (3, 7)
        bm = mod.build_base_path_map(ds, rp)
        self.assertFalse(
            any(
                p.origin_airport_id == "A1" and p.mission_arrival_slot < 3
                for p in bm.path_records
            )
        )

    def test_departure_before_window_is_allowed_when_arrival_is_on_time(self):
        ds, rp = self._input()
        ds["static"]["missions"][0]["_duty_window"] = (3, 7)
        bm = mod.build_base_path_map(ds, rp)
        row = next(
    ...
    def test_window_start_does_not_add_finish_by_deadline_constraint_yet(self):
        ds, rp = self._input()
        ds["static"]["missions"][0]["_duty_window"] = (1, 3)
        ds["static"]["missions"][0]["tau_work"]["fighter"] = 2
        bm = mod.build_base_path_map(ds, rp)
        self.assertTrue(
            any(
                p.mission_arrival_slot < 3
                and p.mission_arrival_slot + p.tau_work_windows > 3
                for p in bm.path_records
            )
        )
